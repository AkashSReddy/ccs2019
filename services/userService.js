...
module.exports.setQuestions = async id => {
  ...
  try {
    ...
    for (var ii = 0; ii < domain.length; ii++) {
      ...
      Sque = await Q_Database.find(
        { qDomain: newDomain, qType: "Subjective" },
        { _id: 1 }
      ).lean();
      Oque = await Q_Database.find(
        { qDomain: newDomain, qType: "Objective" },
        { _id: 1 }
      ).lean();
...
module.exports.validate = userDetails => {
  try {
    ...
    // 18938173831
    // 18[A-Z]{3}[0-9]{3}[0-9]$
    if (!/19[A-Z]{3}[0-9]{3}[0-9]$/.test(regno)) {
